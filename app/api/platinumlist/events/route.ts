...
export async function GET(req: Request) {
  ...
  const data = await service.getEvents({
    ...
    "city.id": params["city.id"] ? Number(params["city.id"]) : 1,
    "nearby_city.id": params["nearby_city.id"] ? Number(params["nearby_city.id"]) : undefined,
    "event_type.id": params["event_type.id"] ? Number(params["event_type.id"]) : undefined,
    event_type_recursive: params.event_type_recursive === "true",
    is_attraction: params.is_attraction ? Number(params.is_attraction) : undefined,
