 ...
 * Fetch all yachts
 */
export async function getAllYachts(params = { page: ['1'], limit: ['20'] }, get_count: boolean = false): Promise<{ data: Yacht[], count?: number }> {
  const { data, error, count } = await getYachts(params)
  if (error) {
 ...
 */
export async function getTodayYachts(
...
export async function getThisWeekYachts(
  params: Record<string, string[]> = (() => {
    const now = new Date()
 ...
 */
export async function getFeaturedYachts(params: Record<string, string[]> = {
