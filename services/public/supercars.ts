import { SupercarPublicRepository } from "@/repository/public/supercars"
import type { Supercar } from "@/schemas/supercar"
import { dummySupercars } from '@/fixtures/dummy-data'



export class SupercarPublicService {
  constructor(private repository: SupercarPublicRepository = new SupercarPublicRepository()) { }
  /**
 * Fetch all supercars
 */
  async getAllSupercars(params = { page: ['1'], limit: ['20'] }, get_count: boolean = false): Promise<{ data: Supercar[], count?: number }> {
    const { data, error, count } = await this.repository.getSupercars(params)
    if (error) {
   ...
   */
  async getFeaturedSupercars(params: Record<string, string[]> = {
